...
class DeadAccountRemover:
    ...
    def updateProgress(self):
        ...
        # print(str(self.accountCountTotal) + " " + str(self.accountProcessedTotal) + " " +str(percentComplete) + "
        # hey!!")
                ...
                if startAtFirstAccount == False:
                    ...
                    for account in line:
                        ...
                        if startAtSpecificAccount:
                            print("Account #: %d" % accountCurrentTotal)
                            if (
                                    accountCurrentTotal == ACCOUNTNUMTOSEARCH):  # accountCurrentTotal != 0 and accountCurrentTotal % 200 == 0):
                                print("Taking a short break...")
                                print(time.time() - startTime)
                                # print("13")
                                # time.sleep(600)
                            ...
                            if title == ['\n                  Page Not Found • Instagram\n                ']:
                                ...
                                self.removedTotal += 1
                                self.app.setLabel("l3", "Total Accounts Removed: " + str(self.removedTotal))
                                self.app.setLabel("l4",
                                                  "Total Accounts Removed In This Batch: " + str(removedCurrentTotal))
                                time.sleep((accountCurrentTotal + 2) % 30)
                            else:
                                self.app.setTextArea("IG Users To Keep", account + ",")
                                # webbrowser.open(urlWithAccount,2)
                            ...
                            # print("15")
                            self.app.setLabel("l5",
                                              "Total Accounts Processed In This Batch: " + str(accountCurrentTotal))
                            self.startAtFirstAccount = False
    ...
    def learnMyFriends(self, file):
        with open(file, 'r') as my_file:
            ...
            for line in read:
                for account in line:
                    if len(account) > 2:
                        message = "Do you know " + account + "?"
                        self.app.setLabel("LMFLabel1", message)
                        self.app.showSubWindow("LMF")
                        if self.isKnownUser:
                            self.app.setTextArea("IG Users To Keep", account + ",")
                        elif not self.isKnownUser and not self.shouldPause:
                            self.app.setTextArea("IG Users Removed", account + ",")
                        elif self.shouldPause:
                            # self.nextAccount = account
...
3. This feature should be used after finding dead accounts")
    ...
    def setup(self):
        ...
        app.startSubWindow("LMF", modal=True, title="Learn My Friends", blocking=True)
        app.setSize(500, 500)
        app.addEmptyLabel("LMFLabel1")
